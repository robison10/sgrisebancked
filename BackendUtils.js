...
const BackendUtils = {
  ...
  createLoginHash: (deviceId, version, timestamp, stumbleId, steamTicket, scopelyId) => {
    const hashInput = `${deviceId}${version}${timestamp}${stumbleId || ''}${steamTicket || ''}${scopelyId || ''}${process.env.LegacySalt}`;
    return crypto.createHash('md5').update(hashInput).digest('hex');
  },
  createRegularHash: (deviceId, googleId, token, timestamp, stumbleId, url, body) => {
    const hashInput = `${deviceId}${googleId}${token}${timestamp}${stumbleId}${url}${body || ''}${process.env.LegacySalt}`;
    return crypto.createHash('md5').update(hashInput).digest('hex');
  },
  ...
  LegacyEncrypt: (text) => {
    const cipher = crypto.createCipheriv('aes-256-cbc', process.env.LegacySalt, crypto.randomBytes(16));
...
class UserModel {
  ...
  static async create(deviceId, platformData = {}) {
    ...
    const username = 'Player ' + BackendUtils.GenCaracters(10).toUpperCase();
    ...
    const user = {
      ...
      balances: [
        ...
        { name: "gems", amount: 5, secondsSince: 0, secondsPerUnit: 0, maxAmount: 0, lastGiven: now },
...
async function generatePhotonJwt(user) {
  ...
  const secret = process.env.LegacySalt;
  const options = { expiresIn: '30d', issuer: 'StumbleRisePhoton' };
...
class UserController {
  static async login(req, res) {
    try {
      ...
      if (!user) {
        Console.log("Login", "criano novo usuario com o deviceId: " + DeviceId);
  ...
  static async updateUsername(req, res) {
    try {
      ...
      if (!/^[a-zA-Z0-9_]+$/.test(Username)) {
        return res.status(422).json({ message: 'Username can only contain letters, numbers, and underscores' });
      }
  ...
  static async linkPlatform(req, res) {
    try {
      ...
      const platformIdMD5 = BackendUtils.Hash('md5', platformId || `${platform}-${user.username}-${process.env.LegacySalt}`);
...
class TournamentXController {
    ...
    userActiveTournaments: [
      {
        ...
        prizeBannerColour: "#E4E407",
        headerColour: "#E4E407",
        mapListGradientColourTop: "#E4E407",
        mapListGradientColourBottom: "#E4E407",
        listPriority: 0,
        minPlayers: 1,
        maxPlayers: 2,
        maxRounds: 1,
        ...
        entryCurrencyCost: 20,
        entryCurrencyType2: "tournament_ticket_legendary",
        entryCurrencyCost2: 1,
        areEmotesRestricted: false,
        prohibitedEmotes: [],
        detailsPanelBorderColourTop: "#E4E407",
        detailsPanelBorderColourBottom: "#FF8300",
        colourData: {
          detailsPanelMainColour: "#E01477",
          detailsPanelBorderColour: "#DA54FF",
          headerGradientRight: "#E01477",
          headerGradientLeft: "#FF63AF",
          infoWidgetsGradientRight: "#8A05E3",
          infoWidgetsGradientLeft: "#6800D2",
          infoWidgetsBorderColour: "#DA54FF"
        },
        rounds: [
          {
            roundOrder: 1,
            maxPlayersToProgress: 1,
            minPlayersPerMatch: 0,
        ...
        awards: [
          {
            placementRangeLowest: 0,
            placementRangeHighest: 0,
            awardId: 0,
            type: "PASSTOKENS",
            amount: 300
          },
          ...
          {
            placementRangeLowest: 3,
            placementRangeHighest: 3,
            awardId: 18,
            type: "CROWNS",
            amount: 1
          }
  ...
  try {
    res.json(this.tournamentData.userActiveTournaments);
  } catch (err) {
    console.error('TournamentX - GetActive Error:', err);
    res.status(500).json({ message: 'Internal server error' });
  }
}
  static async join(req, res) {
    try {
      ...
      const tournament = this.tournamentData.userActiveTournaments.find(
      ...
      const updatedUser = {
        ...user,
        tournaments: [...(user.tournaments || []), tournamentData]
      };
      ...
      console.error('TournamentX - Join error:', err);
      res.status(500).json({ message: 'Internal server error' });
    }
  }
  ...
  static async leave(req, res) {
    try {
      ...
      const updatedTournaments = (user.tournaments || []).filter(
      ...
      console.error('TournamentX - Leave error:', err);
      res.status(500).json({ message: 'Internal server error' });
    }
  }
}
